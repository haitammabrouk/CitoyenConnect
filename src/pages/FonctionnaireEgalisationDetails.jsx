...
function FonctionnaireEgalisationDetails() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8080/DemandeEgalisation/${id}`)
            .then((response) => {
    ...
    useEffect(() => {
        if (conformite) {
            ...
            const decodedContentsecond = Uint8Array.from(atob(conformite.carte), c => c.charCodeAt(0));
    ...
    const openCarte = (e) => {
        ...
        const decodedContent = atob(conformite.carte);
        const documentBlob = new Blob([decodedContent], { type: 'application/pdf' });

        // Create a URL for the Blob
        const documentUrl = URL.createObjectURL(documentBlob);
        // Set the PDF URL in state
        setCarteUrl(documentUrl);
    };
            ...
            <div className="w-full">
                ...
                <div className="page-content font-cairo flex justify-center mt-10">
                    <div className="form w-9/12 shadow-xl rounded-sm pb-10">
                        {/* Form header */}
                        <h1 className="text-center text-3xl font-semibold text-[#336C4E]">Demande D'Egalisation</h1>
                            ...
                            <form className="pt-8" action="" method="post">
                                ...
                                <div className="prenom">
                                    ...
                                    <input  value={conformite.email} className="border border-[#B0B0B0] h-12 w-96 pl-2 rounded-3xl outline-none" type="text" id="email" name="email" disabled />
                                            ...
                                            <button className='hidden' onClick={openCarte}>Ouvrir la carte national</button>
                ...
                <div className='flex justify-between'>
                    <div className='pl-16'>
                        {pdfUrl && <PdfViewer fileUrl={pdfUrl} text={"Voir Document"} />}
                    </div>
                    <div className='pr-36'>
                    {carteUrl && <PdfViewer fileUrl={carteUrl} text={"Voir Carte National"} />}
                    </div>
                </div>
...
export default FonctionnaireEgalisationDetails;
